...
def graph_comp(boss,spe_list): # Grapher un boss avec une compo
    shortest_x(spe_list)
    x,y = reajust_dmg(*boss[:-1],spe_list)
    n = get_name_comp(spe_list)
    c = get_color_comp(spe_list)
    plt.plot(y,marker='o',label=n,color=c)
    return
...
for i in range(5): # Test de plusieurs compo mélange scourge et bsw
    graph_comp(Boss,[pBsw]*(10-i*2)+[cScrg]*i*2)
